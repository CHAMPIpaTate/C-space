...
#define CS_LOW()	SD_CS_SetLow()		/* Set CS low */
#define	CS_HIGH()	SD_CS_SetHigh()		/* Set CS high */
#define	IS_CS_LOW	!SD_CS_GetValue()		/* Test if CS is low */
#define	FORWARD(d)			/* Data forwarding function (console-out in this example) */


static void init_spi (void)		/* Initialize SPI port */
{
    // ATTENTION : choisir SPI mode 0 !!
	SPI1_Initialize();
    SPI1_Open(HOST_CONFIG);
}

/* Delay 100 microseconds */
#define dly_100us() do {__delay_us(100);} while(0);
...
//    dummy = SPI1_ByteRead(); /* Discard received byte */
...
// Decomposition en 2 pour eviter un appel recursif
...
DSTATUS disk_initialize (void)
{
	...
	ty = 0;
    command_ret = send_cmd(CMD0, 0);
	if (command_ret == 1) {			/* Enter Idle state */
